...
export default class Server {
	...
	private registerRoutes(): void {
		...
		this.express.get("/datasets", this.list);
	}
	//start of code generated by ChatGPT
	private add = async (req: Request, res: Response): Promise<void> => {
		//end of code generated by ChatGPT
		let kindParameter: InsightDatasetKind;

		if (req.params.kind === "rooms") {
			kindParameter = InsightDatasetKind.Rooms;
		} else if (req.params.kind === "sections") {
			kindParameter = InsightDatasetKind.Sections;
		} else {
			res.status(400).json({ error: "Something wrong with kind of Dataset." });
			return;
		}
		//start of code generated by ChatGPT
		try {
			const stringFile = req.body.toString("base64");
			const response = await this.insightFacade.addDataset(req.params.id, stringFile, kindParameter);
			res.status(200).json({ result: response });
		} catch (err) {
			res.status(400).json({ error: err + ":Something wrong with the add Dataset." });
		}
		//end of code generated by ChatGPT
	};
	...
	private delete = async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (err instanceof InsightError) {
				res.status(400).json({ error: err + ":Something wrong when remove the dataset" });
			}
			if (err instanceof NotFoundError) {
				res.status(404).json({ error: err + ":Can not find the dataset that need to be remove" });
			}
		}
	};

	private query = async (req: Request, res: Response): Promise<void> => {
		try {
			const response = await this.insightFacade.performQuery(req.body);
			res.status(200).json({ result: response });
		} catch (err) {
			res.status(400).json({ error: err + ":Something wrong with the query." });
		}
	};
	//start of code generated by ChatGPT
	private list = async (_req: Request, res: Response): Promise<void> => {
		//end of code generated by ChatGPT
